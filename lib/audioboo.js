...
AudioBoo.prototype.uploadAudio = function(fileAudio, fileImage, callback){
	var self = this;
	getFile(fileImage, function(err, image){
		
		getFile(fileAudio, function(err, audio){
			//build up the body
			...
			body += "Content-Type: audio/mpeg" + lineReturn;
			body += lineReturn;
			//add the file contents
			body += audio.toString(dataFormat);
			body += lineReturn;
			...
			if(image){
				...
				body += "Content-Type: image/png" + lineReturn;
				body += lineReturn;
				//add the file contents
				body += image.toString(dataFormat);
				body += lineReturn;
			}
			...
			var data = {
				"oauth_body_hash" : self.oa._createSignature(body, self.oauth_access_token_secret)
			}
			
			console.log(data.oauth_body_hash, self.oauth_access_token_secret);
			
			//perform the request
			self.oa._performSecureRequest( self.oauth_access_token_key, self.oauth_access_token_secret, "POST", HOST + "/account/audio_clips.json", data, body, contentType
				,function(error, data, response){
